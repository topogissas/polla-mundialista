export default function ReglasView() {
  ...
  const h3 = { color: '#2A398D', fontSize: '.95rem', marginBottom: 8 } as React.CSSProperties;
  const row = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid #D5D9EB' } as React.CSSProperties;
  ...
  return (
    <div style={{ paddingTop: 12 }}>
      <div style={card}>
        ...
        <div>
          {[
            ['🎯 Marcador exacto', '#2A398D', '25 pts'],
            ['✅ Acertaste el ganador (o empate)', '#3CAC3B', '15 pts'],
            ['❌ Fallaste', '#cfd8d2', '0 pts'],
          ].map(([label, color, pts]) => (
            <div key={label as string} style={row}><span>{label}</span><span style={tag(color as string)}>{pts}</span></div>
          ))}
        </div>
        <p style={{ marginTop: 12, fontSize: '.82rem', color: '#474A4A' }}>Mismo puntaje en <b>todas las fases</b>.</p>
      </div>
      <div style={card}>
        <h3 style={h3}>ℹ️ Cómo funciona</h3>
        <p style={{ fontSize: '.85rem', color: '#474A4A' }}>
