  ...
  return (
    <div id="adminModal" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', display: 'none', alignItems: 'center', justifyContent: 'center', padding: 20, zIndex: 100 }}
      ...
      <div style={{ background: '#fff', borderRadius: 16, padding: 24, maxWidth: 380, width: '100%' }}>
        <h2 style={{ fontSize: '1.15rem', marginBottom: 6, color: '#2A398D' }}>Modo administrador</h2>
        <p style={{ fontSize: '.85rem', color: '#474A4A', marginBottom: 14 }}>Escribe la clave para cargar los resultados reales.</p>
        <input
          ref={passRef}
          type="password"
        ...
        />
        <button onClick={validar} style={{ display: 'block', width: '100%', padding: 13, background: '#2A398D', color: '#fff', border: 0, borderRadius: 12, fontSize: '.95rem', fontWeight: 700, cursor: 'pointer', marginBottom: 8 }}>Activar</button>
        <button onClick={() => { document.getElementById('adminModal')!.classList.remove('show'); }} style={{ display: 'block', width: '100%', padding: 13, background: '#fff', color: '#2A398D', border: '1px solid #3CAC3B', borderRadius: 12, fontSize: '.95rem', fontWeight: 700, cursor: 'pointer' }}>Cancelar</button>
      </div>
